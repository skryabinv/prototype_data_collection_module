...
bool Bmp390::isDataReady(const bmp3_status& status) {
  return (status.intr.drdy == BMP3_ENABLE) || (status.sensor.drdy_press == BMP3_ENABLE) ||
...
int8_t Bmp390::i2cRead(uint8_t reg_addr, uint8_t* reg_data, uint32_t len, void* intf_ptr) {
  ...
  const HAL_StatusTypeDef status =
      HAL_I2C_Mem_Read(&sensor->mHi2c, static_cast<uint16_t>(sensor->mAddress << 1), reg_addr,
                       I2C_MEMADD_SIZE_8BIT, reg_data, static_cast<uint16_t>(len), kI2cTimeoutMs);
...
int8_t Bmp390::i2cWrite(uint8_t reg_addr, const uint8_t* reg_data, uint32_t len, void* intf_ptr) {
  ...
  const HAL_StatusTypeDef status = HAL_I2C_Mem_Write(
      &sensor->mHi2c, static_cast<uint16_t>(sensor->mAddress << 1), reg_addr, I2C_MEMADD_SIZE_8BIT,
      const_cast<uint8_t*>(reg_data), static_cast<uint16_t>(len), kI2cTimeoutMs);
