...
public:
    ...
    /// Следующее измерение запускает вызывающий код через startMeasurement().
...
private:
    ...
    Status writeRegister(std::uint8_t reg, std::uint8_t value);
    Status readRegister(std::uint8_t reg, std::uint8_t* value);
    Status performSetReset();
